...
* **2レイアウトビュー（リスト & ボード）**
  * **リストビュー**: ステータスごとのアコーディオン開閉、期限のカラーアラート、インラインでのタスク即時追加（入力してEnter）など、Asana特有 of 高速インプットを再現。
...
graph TD
    ...
    UI -->|JSON リクエスト| NextAPI
    NextAPI -->|Prismaクエリ| Prisma
    Prisma -->|ポート 5432 / 6543| SupaDB
    SupaDB -->|データ返却| Prisma
    Prisma -->|JSON レスポンス| UI
```
...
```env
DATABASE_URL="postgresql://postgres:[PASSWORD]@db.yakqeblvqcpztgwbofeo.supabase.co:5432/postgres"
DIRECT_URL="postgresql://postgres:[PASSWORD]@db.yakqeblvqcpztgwbofeo.supabase.co:5432/postgres"
```
