...
1. **地理的遅延をカバーするフロントエンド設計**
   * Supabaseへの通信に往復数百ミリ秒〜秒単位の時間がかかる問題に対し、単にローディングを表示して待たせるのではなく、**楽観的UI更新 (Optimistic UI)** とAPIリクエストの非同期スレッド化を行うことで、「ネットワーク遅延をUIで解決する」プロ仕様の設計手法を提示。
2. **Prismaの接続管理（Transaction vs Session Mode）＆ IPv4/IPv6の互換性対策**
